...
namespace Org.BouncyCastle.Crypto.Utilities
{
	...
	{
		// Token: 0x06002A68 RID: 10856 RVA: 0x000E3930 File Offset: 0x000E3930
		public static AlgorithmIdentifier GenerateEncryptionAlgID(DerObjectIdentifier encryptionOID, int keySize, SecureRandom random)
		{
			...
			if (encryptionOID.Equals(PkcsObjectIdentifiers.DesEde3Cbc) || encryptionOID.Equals(AlgorithmIdentifierFactory.IDEA_CBC) || encryptionOID.Equals(OiwObjectIdentifiers.DesCbc))
			{
				byte[] array2 = new byte[8];
				random.NextBytes(array2);
				return new AlgorithmIdentifier(encryptionOID, new DerOctetString(array2));
			}
			if (encryptionOID.Equals(AlgorithmIdentifierFactory.CAST5_CBC))
			{
			...
			if (encryptionOID.Equals(PkcsObjectIdentifiers.RC2Cbc))
			{
				...
				RC2CbcParameter parameters2 = new RC2CbcParameter((int)AlgorithmIdentifierFactory.rc2Table[128], array4);
		...
		private static readonly short[] rc2Table = new short[]
		{
			189,
			86,
			234,
